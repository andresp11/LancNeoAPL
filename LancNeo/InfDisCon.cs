...
namespace LancNeo
{
    public partial class InfDisCon : Form
    {
        ...
        private void InfDisCon_Load(object sender, EventArgs e)
        {
        ...
        private void experiencia()
        {
            ...
            //{
            sqlDABusExpCar.SelectCommand.Parameters["@IdConcretera"].Value = cmbConcretera.SelectedValue;
        ...
        {
            if (dtpIni.Value.DayOfWeek.ToString() == "Monday")
            {
                ...
                for (int Muest = 0; Muest < dsConCarta1.Obra.Rows.Count; Muest++)
                {
                    try
                    {
                        ...
                        {
                            sqlDADatosT.SelectCommand.Parameters["@IdObra"].Value = "%";
                            ...
                            sqlDADatosT.SelectCommand.Parameters["@IdConcretera"].Value = cmbConcretera.SelectedValue;
                            sqlDADatosT.SelectCommand.Parameters["@IdPlanta"].Value = cmbPlanta.SelectedValue;
                ...
                //for (int i = 0; i < dsGraDis1.Muestras.Count; i++)
                //{
                //    dia = int.Parse(dsGraDis1.Muestras.Rows[i][4].ToString());
                //   if (dia > 6 && dia < 14)
                //        dia = dia - 1;
                //    else if (dia > 13 && dia < 21)
                //        dia = dia - 2;
                ...
                //    else if (dia > 27 && dia < 35)
                //        dia = dia - 4;
                //   enes[reeAnt, dia] = enes[reeAnt, dia] + 1;
                //}
                for (int j = 0; j < 8; j++)
                {
                    nt = 0;
                ...
                for (int j = 29; j >= 0; j--)
                    if (ta[j] == 0)
                        curve.Points.Remove(j);
                curve = myPane.AddCurve("14 días", xx, ua, Color.Green, SymbolType.Star);
                curve.Line.Width = 2.5F;
                ...
                */
                ...
                cryInfDisTot1.SetParameterValue("@Cra", " ");
                cryInfDisTot1.SetParameterValue("@Obr", cmbConcretera.SelectedValue);
                cryInfDisTot1.SetParameterValue("@Zon", " ");
                cryInfDisTot1.SetParameterValue("@Dir", " ");
                cryInfDisTot1.SetParameterValue("@Fecha", dtpIni.Value.AddDays(20));
                cryInfDisTot1.SetParameterValue("ObrOCon", true);
            ...
            else if (lunes == 0)
            {
                MessageBox.Show("Elección incorrecta del día de la semana." +
                    System.Environment.NewLine + System.Environment.NewLine, "Elija un lunes",
        ...
        {
//            string dia = dtpIni.Value.dDayOfWeek.ToString();
            if (dtpIni.Value.DayOfWeek.ToString() == "Monday")
            {
                dtpFin.Value = dtpIni.Value.AddDays(33);
                dsConCarta1.Clear();
                sqlDAConcretos.SelectCommand.Parameters["@IdObra"].Value = "%";
                sqlDAConcretos.SelectCommand.Parameters["@Cuantas"].Value = txtMinimo.Text.Trim();
                sqlDAConcretos.SelectCommand.Parameters["@FechaIni"].Value = dtpIni.Value.ToShortDateString();
                sqlDAConcretos.SelectCommand.Parameters["@FechaFin"].Value = dtpFin.Value.ToShortDateString(); ;
                sqlDAConcretos.SelectCommand.Parameters["@IDConcretera"].Value = cmbConcretera.SelectedValue;
                sqlDAConcretos.SelectCommand.Parameters["@IdPlanta"].Value = cmbPlanta.SelectedValue;
                sqlDAConcretos.Fill(dsConCarta1, "Obra");
                dgConcreto.Visible = true;
                dgExperiencia.Visible = true;
                crvGraDis.Visible = false;
        ...
        private void cmbConcretera_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            sqlDAConcretos.SelectCommand.Parameters["@IdPlanta"].Value = cmbPlanta.SelectedValue;
            sqlDAConcretos.Fill(dsConCarta1, "Obra");
            dsPlanta1.Planta.Clear();
            dsPlanta1.Planta.AddPlantaRow(-1, "Todas", (cmbConcretera.SelectedValue.ToString() == "Todas" ? "%" : cmbConcretera.SelectedValue.ToString()));
            sqlDAPLanta.SelectCommand.Parameters["@IdConcretera"].Value = (cmbConcretera.SelectedValue.ToString() == "Todas" ? "%" : cmbConcretera.SelectedValue.ToString().Trim()+"%");
            sqlDAPLanta.Fill(dsPlanta1, "Planta");
            cmbPlanta.SelectedIndex = 0;
            ...
            dgExperiencia.Visible = true;
            crvGraDis.Visible = false;
            if (dsConCarta1.Obra.Rows.Count> 0) experiencia();
        }
